...
export default function History() {
  ...
  return (
    <HistorySection onClick={(e) => e.stopPropagation()}>
      <HistoryBgWrapper initialSlide={0} effect={'fade'} slidesPerView={1} modules={[EffectFade]} ref={swiperRef} centeredSlides>
        ...
        {historyData.map((slideContent, index) => {
          return (
            <SwiperSlide key={index}>
              <HistoryCard
                hj={(hj == 'h' && index % 2 == 0) || (hj == 'j' && index % 2 != 0)}
                point={historyData[0].year === slideContent.year}
                cardBg={slideContent.cardBg}
              >
                {slideContent.content.map((content, idx) => {
                  return (
                    <HistorySubs point={historyData[0].year === slideContent.year} key={idx}>
                      {content}
                    </HistorySubs>
                  );
                })}
