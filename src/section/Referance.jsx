...
export default function Referance() {
  ...
  return (
    <ReferanceWrapper>
      <BgSlide slidesPerView={1} centeredSlides={true} initialSlide={0} effect={'fade'} loop={true} modules={[EffectFade]} ref={swiperRef}>
        {referanceData.map((bg, idx) => {
          return (
            <SwiperSlide key={idx} virtualIndex={activeIndex}>
              {bg.bg ? (
                <ReferImage bg={bg.bg} />
              ) : (
                <ReferVideo width={1000} onClick={(e) => e.stopPropagation()} autoPlay muted playsInline loop preload="metadata">
                ...
                return (
                  <SwiperSlide key={idx}>
                    <SlideItem key={idx}>
                      ...
                      {!isDesktop && data.subsEnter ? (
                        <ItemSubs dangerouslySetInnerHTML={{ __html: data.subsEnter }}></ItemSubs>
                      ) : (
