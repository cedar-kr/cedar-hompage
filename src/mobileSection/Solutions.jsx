...
export default function Solution() {
  return solutionMobileData.map((data, id) => {
    return (
        ...
        <SolutionBox>
          ...
          <SolutionContents rowHeight={data.rowHeight}>
            {data.contents.map((content, idx) => {
              return (
                <ContentBlock key={idx} bg={content.type === 'image' ? content.src : null} grid={content.grid}>
                  <ContentBlockTitle
                    color={
                      content.type === 'image' && content.name !== '키오스크' && content.name !== 'OTP 로그인' && content.name !== '그룹관리' ? 'true' : 'false'
                    }
                    center={content.name == '키오스크'}
